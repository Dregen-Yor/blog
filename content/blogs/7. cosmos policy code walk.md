...
其中 demo 用于训练 Policy Model,world_model_sample 用于训练 World Model,value_function_sample 用于训练 Value Function。
...
denoise 中设置了条件帧不去噪，只去噪后面需要去噪的几帧,根据目标不同有不同设置。

### modules/hybrid_edm_sde.py

这里定义了噪声采样模型，采用了 70% 纯 log-normal，30% uniform的方式，注意这里的 70% 和 30% 是期望。

### modules/cosmos_sampler.py

定义了去噪模型，将 model 的 denoise 函数包装在 x0_fn 中进行调用，去噪 num_step-1 步之后返回结果，对于 num_step == 1 的情况有特殊处理。
## 配置与训练入口
